...
# Provisions the Authentik role/database in the shared PostgreSQL cluster
...
APP_NAMESPACE="${APP_NAMESPACE:-authentik}"
APP_SECRET="${APP_SECRET:-authentik-credentials}"
APP_DB="${APP_DB:-authentik}"
...
info "Reading Authentik DB credentials from secret ${APP_NAMESPACE}/${APP_SECRET}"
...
pass "Loaded Authentik app credentials"
...
info "Verifying login with Authentik application credentials"
kubectl exec -i -n "$PG_NAMESPACE" "$TOOL_POD" -- env \
...
pass "Authentik credentials can log in to ${APP_DB}"

echo ""
echo "Provisioning completed successfully."
echo "Next: sync the authentik app so the deployment picks up the provisioned database and secret."