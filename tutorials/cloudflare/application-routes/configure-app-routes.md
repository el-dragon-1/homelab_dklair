...
## Protecting LocalAI with Zero Trust Access (MFA)

Use this workflow to protect `localai.dklair.io` behind Cloudflare Access so only authenticated users can reach LocalAI.

### Prerequisites

1. LocalAI ingress exists in the cluster and resolves externally at `localai.dklair.io`.
2. A Cloudflare Tunnel is already publishing the hostname route.
3. At least one Identity Provider is configured in Zero Trust (Google, GitHub, Okta, Azure AD, etc.).
4. MFA is enforced at the Identity Provider level for the users/groups that will access LocalAI.
...
1. Go to Networks > Tunnels.
2. Select your tunnel and open Public Hostnames.
3. Confirm a hostname route exists for `localai.dklair.io`.
4. Ensure it forwards to the correct origin service (your cluster ingress endpoint).
...
3. Choose Self-hosted.
4. Set:
	- Application name: LocalAI
	- Domain: `localai.dklair.io`
	- Session duration: short (for example 8h or less)
5. Save and continue to policies.
...
1. Do not add broad bypass rules for this app.
2. Do not expose LocalAI without Access policies.

### Step 4: Test Access Flow

1. Open `https://localai.dklair.io` in a private/incognito browser window.
2. Confirm you are redirected to Cloudflare Access login.
3. Sign in with an allowed identity.
4. Confirm your IdP enforces MFA during sign-in.
5. Confirm LocalAI loads only after successful authentication.
...
1. This repository does not currently manage Cloudflare Access policies via GitOps, so the Access application and policy changes are made in Cloudflare UI.
2. Cluster-side ingress for LocalAI is managed in [values/local-ai/values.yaml](values/local-ai/values.yaml).