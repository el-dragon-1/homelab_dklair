...
- For a new protected app behind Traefik forward-auth, reuse the embedded outpost and create one Forward auth (single application) Provider plus one Authentik Application per hostname; reuse groups and policies when the access rules are shared.
...
## Deployment Workflow
- Start in Authentik Admin with `Applications -> Providers` to create or reuse the Forward auth (single application) Provider.
